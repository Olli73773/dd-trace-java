...
public class InjectRequestContextVisitor extends ClassVisitor {
  ...
  public static ClassVisitor createVisitor(ClassVisitor cv, MethodList<?> methods) {
    ...
    return new InjectRequestContextVisitor(cv, methodList, hasOnEnter, hasOnExit);
  }

  public InjectRequestContextVisitor(
      ClassVisitor cv, List<MethodDescription> methodList, boolean hasOnEnter, boolean hasOnExit) {
  ...
  public MethodVisitor visitMethod(
    ...
    return new AdviceMethodVisitor(this.api, methodDescription, this.adviceContent, mv);
  }
...
class AdviceMethodVisitor extends MethodVisitor {
  ...
  public void visitEnd() {
    if (shouldFetchReqContext()) {
      ...
      suppressSorter(
          new Runnable() {
            @Override
            public void run() {
              buildParameterFrame(new Object[] {Type.getInternalName(AgentSpan.class)});
            }
          });
    ...
    if (this.addReqCtxParam) {
      ...
      sw.visitClassType(Type.getInternalName(RequestContext.class));
